...
def grid_search(model = None, hrir_all = None, verbose=0, num_epochs = 100):
    ...
    for pair in pairs:
        ...
        print("{}, {}: {:.4f}; Holdout: {}; current best is [({},{}), ({}, {})] with {:.4f}".format(pair[0], pair[1], loss, holdout_num, int(best_pair[0][0]), int(best_pair[0][1]), int(best_pair[1][0]), int(best_pair[1][1]), best_loss))
